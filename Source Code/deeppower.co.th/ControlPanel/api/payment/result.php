...
<?php  
	...
	$sql = "select CheckOutCode from checkout where CheckOutCode=:CheckOutCode ";
    $data = SelectRow($sql, array('CheckOutCode' => $_REQUEST["order_id"]));
    if($data === false)
    {
	...
	try {
		ExecuteSQL("insert into checkout_log(CheckOutCode,Status,StatusDesc,ResultData,CreatedOn) 
			VALUES (:CheckOutCode, :Status, :StatusDesc, :ResultData ,now())"
		, array(
			'CheckOutCode' => $order_id,
			'Status' => $payment_status,
			'StatusDesc' => $channel_response_desc,
			'ResultData' => json_encode($_REQUEST,JSON_UNESCAPED_UNICODE),
		));
...
if(strcmp(strtolower($hash_value), strtolower($checkHash))==0){
	...
	$sql = " select CheckOutCode,StatusCode from checkout where CheckOutCode = :CheckOutCode ";
    $data = SelectRow($sql, array('CheckOutCode' => $order_id));
    if($data === false)
    {
        Redirect($GLOBALS["ROOT"]."/".$_Util_PageConfig->GetConfig("HOME")->PageURLName());
        exit();
	...
	$sqlUpdate = "
		update checkout set 
			...
			,UpdatedOn=now()
			,Pay123_payment_status= :Pay123_payment_status
			,Pay123_request_timestamp= :Pay123_request_timestamp
			,Pay123_merchant_id= :Pay123_merchant_id
			,Pay123_transaction_datetime= :Pay123_transaction_datetime
			,Pay123_paid_channel= :Pay123_paid_channel
			,Pay123_paid_agent= :Pay123_paid_agent
