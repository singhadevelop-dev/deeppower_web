...
<?php 
    ...
    $sql = "select CheckOutCode,Total,Tax,DeliveryPrice,Net,ClientID,Email,Name from checkout where CheckOutCode=:CheckOutCode and StatusCode=:Status  ";
    $data = SelectRow($sql, array('CheckOutCode' => $refCode, 'Status' => 'WAITING'));
    if($data === false)
    {
        Redirect("../../../");
        exit();
    ...
    $sqlProduct = "select p.*
                    ...
                    where cart.CheckOutCode= :CheckOutCode
                        order by p.seq,p.ProductCode,cart.SEQ";
    $dataPrd = SelectRowsArray($sqlProduct, array('CheckOutCode' => $data["CheckOutCode"]));
    $productArr = array();
