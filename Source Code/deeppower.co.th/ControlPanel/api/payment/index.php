...
function PaymentPOClient($clientCode,$data)
{
    ...
    $datas = SelectRowsArray("select * from checkout where CheckOutCode = :CheckOutCode and ClientID = :ClientID ", array(
    ...
    $uploadFileTarget =  $GLOBALS["ROOT"]."/_content_images/".GetCurrentLang()."/".strtolower($checkOutCode)."/";
    ...
    ExecuteSQL($sqlInsert, array(
        ...
        'checkOutCode' => $checkOutCode,
        'BankCode' => $ddlBankCode,
        'Date' => $txtDate,
        'Time' => $txtTime,
        'Amount' => $txtAmount,
        'Picture' => $fileUploadedPath,
        'Address' => $Address
    ));
    $_payment_date = ConvertDateTimeDisplayToDateTimeDB($paymentDateTime.":00");
    ExecuteSQL("update checkout set StatusCode='CHECKPAID' , PaymentDate = :PaymentDate where CheckOutCode= :CheckOutCode "
        , array('PaymentDate' => $_payment_date, 'CheckOutCode' => $checkOutCode));
