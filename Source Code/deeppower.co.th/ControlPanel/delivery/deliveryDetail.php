...
if (isset($_POST["btnSave"])) {
    ...
    if (empty($_GET["ref"])) {
        ...
        $mainparamset = array('genID' => $genID, 'txtSubject' => $txtSubject, 'chkActive' => $chkActive, 'usercode' => UserService::UserCode());
    ...
    ";
        ...
        $mainparamset = array('code' => $_GET["ref"], 'txtSubject' => $txtSubject, 'chkActive' => $chkActive, 'usercode' => UserService::UserCode());
    ...
    ExecuteSQLTransaction($sql, $mainparamset, "index.php");

    $sql = "delete from delivery_price where Code = '$Code'";
    ExecuteSQL($sql);
    for ($i = 0; $i < count($_POST["weight"]); $i++) {
        $samepro = (!empty($_POST["PROVICE_SAME"][$i])) ? "" : $_POST["PROVICE_SAME"][$i] ;
        $subparam = array('code' => $Code, 'weight' => $_POST["weight"][$i], 'bkk' => $_POST["BKK"][$i], 'same' => $samepro, 'diff' => $_POST["PROVINCE_DIFF"][$i]);
        ...
        ExecuteSQL($sql, $subparam);
    }
}

if (!empty($_GET["ref"])) {
    $sql = "select * from delivery_category where Code = :code";
    $data = SelectRow($sql, array('code' => $_GET["ref"]));
}
?>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <!-- <td class="hide">
                                    <input required name="PROVINCE_SAME[]" value="<?php //echo $d["PROVINCE_SAME"] 
                                                                                    ?>" type="number" class="PROVINCE_SAME form-control text-right" step="1" />
                                </td> -->
                                <td>
