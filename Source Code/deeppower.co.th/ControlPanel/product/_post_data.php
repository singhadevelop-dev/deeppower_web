...
{
    $new = $data["new"] == "true" ? 1 : 0;
    $sql = "update product set Hot = :Hot where ProductCode = :ProductCode ";
    ExecuteSQL($sql,array('Hot' => $new,'ProductCode' => $data["product"]));
    return $data;
}
...
function updateActive($data)
{
    ...
    $sql = "update product set Active = :Active where ProductCode = :ProductCode ";
    ExecuteSQL($sql,array('Active' => $active,'ProductCode' => $data["product"]));
    return $data;
}
