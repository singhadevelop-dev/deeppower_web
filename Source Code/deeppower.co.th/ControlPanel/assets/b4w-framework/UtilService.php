...
class UserService
{
    ...
    // {
    //     $result = SelectRow("select Password from user where UserCode = :userCode", array('userCode' => $UserCode));
    ...
    public static function _CheckPassword($UserCode,$password,$isCustomer = false)
    {
        if($isCustomer)
        {
            $result = SelectRow("select Password from client where ClientCode = :userCode",array('userCode' => $UserCode));
        }else
        {
            $result = SelectRow("select Password from user where UserCode = :UserCode",array('UserCode' => $UserCode));
        }
    ...
    public static function _UserResetPassword($username,$type){
        if($type == "CUSTOMER"){
            $result = SelectRow("select * from customer_contact where Email = :username", array('username' => $username));
            if($result !== false){
                $newPassword = uniqid();
                $newPasswordGenerate = GeneratePassword($newPassword);
                ExecuteSQL("update customer_contact set Password = :Password where ContactCode = :ContactCode"
                    , array('Password' => $newPasswordGenerate, 'ContactCode' => $result["ContactCode"]));
            ...
            $result = SelectRow("select * from employee where Username = :username", array('username' => $username));
            if($result !== false){
                $newPassword = uniqid();
                $newPasswordGenerate = GeneratePassword($newPassword);
                ExecuteSQL("update employee set Password = :Password where EmployeeCode = :EmployeeCode "
                    , array('Password' => $newPasswordGenerate, 'EmployeeCode' => $result["EmployeeCode"]));
                SendEmailResetPassword(
                    $result["FirstName"]." ".$result["LastName"],
                    $result["Email"],
                    $result["Username"],
    ...
    {
        $result = SelectRow("select * from user where Active = 1 and UserName = :username", array('username' => $username));
        if($result !== false){
    ...
    {
        $result = SelectRow("select * from client where Active = 1 and (UserName = :UserName or Email = :Email or Phone = :Phone)",
                array(
                    'UserName' => $username,
                    'Email' => $username,
                    'Phone' => $username
...
function GenerateNextID($table,$key,$length,$prefix)
{
    ...
    $datas = SelectRow("select $key from `$table` where $key like '$prefix%' order by $key desc limit 1", array());
    if(!$datas){
...
function SelectRow($sql, $paramsSet)
{
    try {
        $dbcon = GetPDOConnection();
        $dbcon->query("SET SQL_BIG_SELECTS=1");
...
function SelectRows($sql, $paramsSet = null, $connection = null)
{
	try {
        $dbcon = $connection == null ? GetPDOConnection() : $connection;
        $dbcon->query("SET SQL_BIG_SELECTS=1");
        $stmt = $dbcon->prepare($sql);
        if($paramsSet !== null)
        {
...
function SelectRowsArray($sql, $paramsSet = null)
{
    return SelectRows($sql, $paramsSet);
}

function ExecuteMultiSQL($sql, $paramsSet = null)
{
...
function ExecuteSQL($sql,$paramsSet = null)
{
    try {
        $dbcon = GetPDOConnection();
        $stmt = $dbcon->prepare($sql);
...
function ExecuteSQLTransaction($strSql, $paramsSet = null, $url)
{
    try {
        $dbcon = GetPDOConnection();
...
function FindDataMutiple($datas_list, $search_list = array(),$isRowSingle = false) { 
    ...
    if($search_list == null || countval($search_list) <= 0)
    {
        return $datas_list;
    }
    $result = array();
...
function GetTicketLogs($ticketCode)
{
    $sql = "select 
        ...
        where l.sid = :sid and l.Type = 'TICKET' 
        and l.RefKey = :refKey order by created_on";
    return SelectRows($sql, array('sid' => UserService::SID(),'refKey' => $ticketCode));
}
...
{
	$datas = SelectRow("select Firstname,Lastname from employee where EmployeeCode = :employee", array('employee' => $empCode));
    if(!$datas){
        return "";
    }
    else{
...
{
	$datas = SelectRow("select ImagePath from employee where EmployeeCode = :employee", array('employee' => $empCode));
    if(!$datas){
...
{
	$datas = SelectRow("select ImagePath from customer_contact where ContactCode = :ContactCode", array('ContactCode' => $contactCode));
    if(!$datas){
        return "/images/user.png";
    }
    else{
...
{
	$datas = SelectRow("select ImagePath from employee where EmployeeCode = :employee", array('employee' => $created_by));
    if(!$datas){
...
function SelectEmployeeFullnameDepartment($sid,$empCode)
{
	$datas = SelectRow("select e.EmployeeCode,CONCAT(e.Firstname,' ',e.Lastname,' (',d.DepartmentName,')') as FullDesc 
                                ...
                                and e.EmployeeCode = :employee", array('employee' => $empCode));
    if(!$datas){
        return "";
    }
    else{
...
function GetHotlineSupporter()
{
    ...
    $hotline = SelectRow("select * from hotline_mapping where SID = :sid
                                and WeekNum=:weekNum 
                                and Year = :year", array('sid' => UserService::SID(), 'weekNum' => $week, 'year' => $curYear));     
    if(!$hotline){
        return "";
    }	
    else{
...
function GetDropDownListOptionWithDefaultSelectedAndCondition($table,$key,$text,$default,$selected,$whereOBJ)
{
    $strResult = ""."<option value=''>$default</option>";
    $where = '';
    $keys = array_keys($whereOBJ);
    $inx = 0;
    foreach ($keys as $k) {
        $inx++;
        if($inx == 1){
    ...
    $results = SelectRows("select * from $table where $where order by $text asc", $whereOBJ);
...
{
	$dateTime = SelectRow("select NOW() as cdate", array());
...
function SendEmailContact($contactCode,$refPO = "")
{
    ...
    $sqlContact = "select * from member where MemberCode = :memberCode";
    $dataContact = SelectRow($sqlContact, array('memberCode' => $contactCode));
    $dataWeb = GetWebsiteDetail();
...
function SendEmailCareer($careerCode)
{
    ...
    $dataContact = SelectRow($sqlContact, array('careerCode' => $careerCode));
    $dataWeb = GetWebsiteDetail();
    $protocol = stripos($_SERVER['SERVER_PROTOCOL'],'https') === true ? 'https://' : 'http://';
    $uri = $protocol.$_SERVER['HTTP_HOST'];
...
function SendEmailContactRefProduct($contactCode,$productDesc,$isQuotation)
{
    ...
    $dataContact = SelectRow($sqlContact, array('contactCode' => $contactCode));
    $dataWeb = GetWebsiteDetail();
    $protocol = stripos($_SERVER['SERVER_PROTOCOL'],'https') === true ? 'https://' : 'http://';
    $uri = $protocol.$_SERVER['HTTP_HOST'];
...
function SendEmailPO($POCode,$subjectMessage)
{
        ...
        <tr>
            ...
            <td valign='top' style='padding-left: 15px;'>
                <h5 style='font-size: 14px; color:#444;margin-top:15px; margin-bottom: 0px;'>
                    Size : <span> {2-1}</span></h5>
                <h5 style='font-size: 14px; color:#444;margin-top:15px; margin-bottom: 0px;'>
                    Color : <span> {2-2}</span> </h5>
                <h5 style='font-size: 14px; color:#444;margin-top: 10px;'>QTY : <span>{4}</span></h5>
            </td>
            <td valign='top' style='padding-left: 15px;'>
                <h5 style='font-size: 14px; color:#444;margin-top:15px'><b>฿ {6}</b></h5>
            </td>
        </tr>";
    $POItems = "";
    $sqlItem = "select p.*,c.Price as CartPrice ,c.QTY,c.Total
                ...
                where c.CheckOutCode = :checkOutCode and c.CheckOutCode <> ''";
    $dataItems = SelectRows($sqlItem, array('checkOutCode' => $POCode));
    ...
    $dataWeb = GetWebsiteDetail();
    $sqlPO = "select * from checkout where CheckOutCode = :checkOut";
    $dataPO = SelectRow($sqlPO, array('checkOut' => $POCode));
    $sqlorder = "select * from portfolio where PortType='MASTERDETAIL-EMAILORDER' and Active=1";
    $dataOrder = SelectRow($sqlorder, array());
    $emailorder = ConvertNewLine($dataOrder["ShortDescription"]);
    $body = file_get_contents($GLOBALS["DOCUMENT_ROOT"]."/ControlPanel/assets/template/Email-Order-Success-1.php");
    ...
    $body = str_replace("[!--EMAILORDER--!]",$emailorder,$body);
    ...
    $body = str_replace("[!--SHOP_IMAGE--!]",$uri.$GLOBALS["ROOT"]."/assets/images/email-temp/delivery-2.png",$body);
    $body = str_replace("[!--SHOP_IMAGE2--!]",$uri.$GLOBALS["ROOT"]."/assets/images/email-temp/space.jpg",$body);
    $body = str_replace("[!--SHOP_CHECK--!]",$uri.$GLOBALS["ROOT"]."/Ordersuccess.php?ref=".base64_encode($POCode),$body);
    ...
    รายละเอียด : ".$uri.$GLOBALS["ROOT"]."/Ordersuccess.php?ref=".base64_encode($POCode)."
    ";
    
    $lineToken = "R1QSJjZewoLcaWIhy17sS1ex7Y2ooBTpc2llX45uuXz";
    SendLineNotification($lineToken,$lineMessage);
}

function SendEmailPODelivery($POCode,$subjectMessage)
{
    //$protocol = stripos($_SERVER['SERVER_PROTOCOL'],'https') === true ? 'https://' : 'http://';
    $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? 'https://' : 'http://';
    $uri = $protocol.$_SERVER['HTTP_HOST'];
    ...
    $sqlPO = "select a.* , b.Name as DeliveryName
        ...
        where a.CheckOutCode = :CheckOutCode";
    $dataPO = SelectRow($sqlPO, array('CheckOutCode' => $POCode));

    $body = file_get_contents($GLOBALS["DOCUMENT_ROOT"]."/ControlPanel/assets/template/Email-Order-Success-1.php");
    
    $body = str_replace("[!--NAME--!]",$dataPO["Name"],$body);
    $body = str_replace("[!--WEB_NAME--!]",$dataWeb["WebName"],$body);
    $body = str_replace("[!--PO_NUMBER--!]",$POCode,$body);
    $body = str_replace("[!--SHIPPING_URL--!]",$dataPO["DelivieryURL"],$body);
    $body = str_replace("[!--SHIPPING_NUMBER--!]",$dataPO["DelivieryCode"],$body);
    $body = str_replace("[!--LIST_ORDER--!]",$POItems,$body);
    $body = str_replace("[!--LOGO--!]",$uri.SystemConfig::cogs_logo_path(),$body);
    $body = str_replace("[!--LOGO_WIDTH--!]",SystemConfig::cogs_logo_width(),$body);
    $body = str_replace("[!--LOGO_HEIGHT--!]",SystemConfig::cogs_logo_height(),$body);
    ...
    $body = str_replace("[!--SHOP_CHECK--!]",$uri.$GLOBALS["ROOT"]."/Ordersuccess.php?ref=".base64_encode($POCode),$body);
    SendMailControlPanel($dataPO["Email"],$body,$subjectMessage,$dataWeb["Email"],"",$dataWeb);
}
...
function SendLineNotification2($code){
    ...
    $uri = $protocol.$_SERVER['HTTP_HOST'];
    $dataWeb = GetWebsiteDetail();
    $sqlPO = "select * from product where ProductCode = :ProductCode ";
    $dataPO = SelectRow($sqlPO, array('ProductCode' => $code));
    $_URL = $uri.$GLOBALS["ROOT"]."/"."ProductDetail.php?ref=".$dataPO["ProductCode"]." ";

    $path_qrcode = "../../_content_images/qrcode/";
	$file_qrcode = $dataPO["ProductCode"].".png";
    ...
    $sToken = "R1QSJjZewoLcaWIhy17sS1ex7Y2ooBTpc2llX45uuXz";

    $sMessage = "เพิ่มหรือแก้ไขข้อมูลบนเว็บไซต์ ".$dataWeb["WebName"]."\n";
    $sMessage .= "เมื่อ: " . ConvertDateTimeDBToDateTimeDisplay(GetCurrentDateTimeServer()) . " \n";
    $sMessage .= "ชื่อรายการ : " . $dataPO["ProductName"] . " \n";
...
function GetPOPaymentMethodDesc($paymentType){
    ...
    else if($paymentType == "Omise")
        return "ชำระผ่าน Omise";
    return $paymentType;
}
...
function GetWebsiteDetail(){
    ...
    $dataWeb = SelectRow($sqlWeb, array());
    return $dataWeb;
}
function SendEmailResetPassword($toName,$toEmail,$userName,$password,$href)
{
    $body = file_get_contents($GLOBALS["DOCUMENT_ROOT"]."/email-template/reset-password-email.html");
    ...
    $body = str_replace("[!--HREFLOGIN--!]",$href,$body);
    SendMail($toEmail,$body,"Reset your password successfully",false,"");
}
...
function SendEmailTicket($mailSubject,$mode,$ticketCode,$trackingCode,$toName,$toEmail,$action,$createdBy,$subject,$symtom,$contactName,$technicianName,$RateCode,$customerName,$message,$attachFilePath)
{
    $enableCcOperation = true;
    $body = "";
    if($mode == "DEFAULT"){
        $body = file_get_contents($GLOBALS["DOCUMENT_ROOT"]."/email-template/ticket-email.html");
        $body = str_replace("[!--ACTION--!]",$action,$body);
        $body = str_replace("[!--SYMTOM--!]",str_replace("\n","<br>",$symtom),$body);
        $body = str_replace("[!--CREATOR--!]",$contactName,$body);
        $body = str_replace("[!--RESPONSE--!]",$technicianName,$body);
        $body = str_replace("[!--TICKETCODE--!]",$ticketCode,$body);
        $body = str_replace("[!--TRACKINGCODE--!]",$trackingCode,$body);
        $body = str_replace("[!--CUSTOMER--!]",$customerName,$body);
        $body = str_replace("[!--MESSAGE--!]",$message,$body);
    }
    else if($mode == "CUSTOMER"){
        $body = file_get_contents($GLOBALS["DOCUMENT_ROOT"]."/email-template/ticket-customer-email.html");
        $body = str_replace("[!--ACTION--!]",$action,$body);
        $body = str_replace("[!--SYMTOM--!]",str_replace("\n","<br>",$symtom),$body);
        $body = str_replace("[!--CREATOR--!]",$contactName,$body);
        $body = str_replace("[!--RESPONSE--!]",$technicianName,$body);
        ...
        $body = str_replace("[!--CUSTOMER--!]",$customerName,$body);
        $body = str_replace("[!--MESSAGE--!]",$message,$body);
        $enableCcOperation = false;
    }
    else if($mode == "COMPLETE"){
        $body = file_get_contents($GLOBALS["DOCUMENT_ROOT"]."/email-template/ticket-complete-email.html");
        $body = str_replace("[!--RATECODE--!]",$RateCode,$body);
        $enableCcOperation = false;
    }
    $body = str_replace("[!--SUBJECT--!]",$subject,$body);
    $body = str_replace("[!--SENDTO_NAME--!]",$toName,$body);
    $body = str_replace("[!--DATETIME--!]",ConvertDateTimeDBToDateTimeDisplay(GetCurrentDateTimeServer()),$body);
    $body = str_replace("[!--SENDER_NAME--!]",$createdBy,$body);
    $body = str_replace("[!--CREATED_ON--!]",ConvertDateTimeDBToDateTimeDisplay(GetCurrentDateTimeServer()),$body);
    SendMail($toEmail,$body,$mailSubject,$enableCcOperation,$attachFilePath);
}
function SendEmailTicketRemindBuilder($ticketCode,$overTime)
{
	$createdBy = " you for more than $overTime. Please sign in and accept your case now ! The reminder e-email was sent ";
    $resultTicket = GetTicketRow($ticketCode);
    $toEmail = array();
    $toName = array();
    array_push($toEmail,$resultTicket["LeaderEmail"]);
    array_push($toName,$resultTicket["LeaderName"]." (Responsible Person)");
    $Precis = "";
    $PrecisDesc = "";
    if(!empty($resultTicket["Precis"])){
        $Precis = " - ".$resultTicket["Precis"];
        $PrecisDesc = " [".$resultTicket["Precis"]."]";
    }
    SendEmailTicket(
        "Warning! ".$resultTicket["TicketCode"].$Precis." : The ticket was opened for more than $overTime.Please check and start support.",
        "DEFAULT",
        $resultTicket["TicketCode"],
        $resultTicket["TrackingCode"],
        join(", ",$toName),
        join(", ",$toEmail),
        "OVERLOOK",
        $createdBy,
        $resultTicket["Subject"],
        $resultTicket["Symptom"],
        $resultTicket["ContactName"],
        $resultTicket["LeaderName"],
        "",
        $resultTicket["CustomerName"].$PrecisDesc,
        "",
        ""
    );
}
function SendEmailTicketBuilder($ticketCode,$action)
{
	$createdBy = UserService::UserFullName()." (".UserService::UserType().")";
    $resultTicket = GetTicketRow($ticketCode);
    $toEmail = array();
    $toName = array();
    array_push($toEmail,$resultTicket["LeaderEmail"]);
    array_push($toName,$resultTicket["LeaderName"]." (Responsible Person)");
    if(!in_array($resultTicket["DupityEmail"],$toEmail)){
        array_push($toEmail,$resultTicket["DupityEmail"]);
        array_push($toName,$resultTicket["DupityName"]." (Actknowledge Person)");
    }
    $Precis = "";
    $PrecisDesc = "";
    if(!empty($resultTicket["Precis"])){
    ...
    SendEmailTicket(
        $resultTicket["TicketCode"].$Precis." : The ticket was ".$action,
        "DEFAULT",
        $resultTicket["TicketCode"],
        $resultTicket["TrackingCode"],
        join(", ",$toName),
    ...
    SendEmailTicket(
        $resultTicket["TrackingCode"]." : The ticket was ".$action,
        "CUSTOMER",
        $resultTicket["TicketCode"],
        $resultTicket["TrackingCode"],
        $resultTicket["ContactName"]." (Case Owner)",
        $resultTicket["ContactEmail"],
        $action,
        $createdBy,
        $resultTicket["Subject"],
        $resultTicket["Symptom"],
...
function SendEmailTicketUpdateBuilder($ticketCode,$message)
{
    $action = "UPDATED";
	$createdBy = UserService::UserFullName()." (".UserService::UserType().")";
    $resultTicket = GetTicketRow($ticketCode);
...
function SendEmailTicketComoleteBuilder($ticketCode,$action)
{
	$createdBy = UserService::UserFullName()." (".UserService::UserType().")";
    $resultTicket = "";//GetTicketRow($ticketCode);
    $toEmail = array();
    ...
    $attachFilePath = "/mpdf/report/complete/Report Complete ".$resultTicket["TicketCode"].".pdf";
    SendEmailTicket(
        $resultTicket["TicketCode"].$Precis." : The ticket was ".$action,
        "DEFAULT",
        $resultTicket["TicketCode"],
    ...
    $RateCode = uniqid(). uniqid(). uniqid();
    ExecuteSQL("update ticket set RateCode = :RateCode where TicketCode = :TicketCode", 
        array('RateCode' => $RateCode, 'TicketCode' => $ticketCode));
    SendEmailTicket(
       $resultTicket["TrackingCode"]." : Your problem is resolved.",
       "COMPLETE",
       $resultTicket["TicketCode"],
       $resultTicket["TrackingCode"],
       $resultTicket["ContactName"]." (Case Owner)",
...
// function GetTicketRow($ticketCode)
// {
// 	return SelectRow("select 
//                         case when cust.CustomerName is null then con.FreeTextCompany else cust.CustomerName end as CustomerName,
//                         cust.Precis,
//                         CONCAT(con.FirstName,' ',con.LastName) as ContactName,
//                         case when con.Email is null then con.EmailCustom else con.Email end as ContactEmail,
//                         CONCAT(leader.Firstname,' ',leader.Lastname) as LeaderName,
//                         leader.Email as LeaderEmail,
//                         CONCAT(dupity.Firstname,' ',dupity.Lastname) as DupityName,
//                         dupity.Email as DupityEmail,
//                         t.* from ticket t
//                         left join customer_contact con on t.SID = con.SID
//                         and t.ContactCode = con.ContactCode
//                         left join employee leader on t.SID = leader.SID
//                         and t.EmployeeCode = leader.EmployeeCode
//                         left join employee dupity on t.SID = dupity.SID
//                         and t.EmployeeDupityCode = dupity.EmployeeCode 
//                         left join customer cust on t.SID = cust.SID
//                         and cust.CustomerCode = con.CustomerCode
//                         where t.TicketCode = '$ticketCode'");
// }
// ================ PUBLIC SITE ======================
function GetAllLanguageArray()
{
	$langArray = array();
    $resultlang = SelectRows("select * from publicsite_language_master where SID = :sid", array('sid' => UserService::SID()));
    foreach ($resultlang as $lang) {
        $langArray[$lang["LanguageCode"]] = array(
...
function GetAllEventArray()
{
	$langArray = array();
    $langArray["NS"] = "News"; 
    $langArray["IA"] = "Internal activities"; 
    $langArray["SN"] = "Seminars"; 
    $langArray["CS"] = "Corporation social responsibility"; 
    return $langArray;
}
function GetEventDetail($key)
{
    $arr = GetAllEventArray();
    return $arr[$key];
}
...
// function GetDynamicContentDetail($generateCode,$langCode){
//     $rest = SelectRow("select text from publicsite_dynamic_content where SID = '".UserService::SID()."' and GenerateCode = '".$generateCode."' and LanguageCode = '".$langCode."'");
//     return $rest["text"];
// }
...
function UploadFile2($file,$path,$FileID = "")
{
    $guid = empty($FileID) ? GetGUID() : $FileID;
    $target_root = $_SERVER["DOCUMENT_ROOT"];
    $target_dir = $path;
...
function ReArrayFiles(&$file_post) {
    $file_ary = array();
    $file_count = countval($file_post['name']);
...
function GenerateHTAccess(){
    ...
    foreach ($dbList as $lang => $db) {
        ...
        $datas = SelectRows($sql, array(), GetPDOConnection($db));
        ...
        $sql = "select a.PortType, a.PageFileName, b.PortCode, b.PortName from page_config_portfolio a
                    inner join portfolio b on a.PortType = b.PortType
                where b.Active = 1 and b.PortName <> '' ";
        $datas = SelectRows($sql, array(), GetPDOConnection($db));
        foreach ($datas as $port) {
            if(!empty($port["PortRefCode"])){
                $urlPart = truncateThaiText($port["PortRefCode"], 50);
                $str .= "\n RewriteRule ^".SanitizeURL($urlPart,true)."?$ ".$port["PageFileName"]."?ref=".$port["PortCode"]." ";
            }else{
                $urlPart = truncateThaiText($port["PortName"], 50);
                $str .= "\n RewriteRule ^".SanitizeURL($urlPart,true)."?$ ".$port["PageFileName"]."?ref=".$port["PortCode"]." ";
            }
            // $str .= "\n RewriteRule ^".SanitizeURL($port["PortName"],true)."-".$port["PortCode"]."?$ ".$port["PageFileName"]."?ref=".$port["PortCode"]." ";
        }

        $sql = "select * from product where ProductName <> ''";
        $datas = SelectRows($sql, array(), GetPDOConnection($db));
        foreach ($datas as $data) {
            // if(!empty($data["ProductRefCode"])){
            //     $str .= "\n RewriteRule ^".SanitizeURL($data["ProductRefCode"],true)."-".$data["ProductCode"]."?$ ProductDetail.php?ref=".$data["ProductCode"]." ";
            // }else{
        ...
        $sql = "select * from product_category where CategoryGroup = 'PRODUCT' and Active = 1 and CategoryName <> ''";
        $datas = SelectRows($sql, array(), GetPDOConnection($db));
        foreach ($datas as $data) {
                $urlPart = truncateThaiText($data["CategoryName"], 50);
                $str .= "\n RewriteRule ^".SanitizeURL($urlPart,true)."?$ Product.php?cat=".$data["CategoryCode"]." ";
        }

        $sql = "select * from product_category where CategoryGroup = 'SERVICE' and Active = 1 and CategoryName <> ''";
        $datas = SelectRows($sql, array(), GetPDOConnection($db));
        foreach ($datas as $data) {
                $urlPart = truncateThaiText($data["CategoryName"], 50);
                $str .= "\n RewriteRule ^".SanitizeURL($urlPart,true)."?$ Service.php?cat=".$data["CategoryCode"]." ";
        }
    }
...
function DetailPageURL2($name)
{
    $urlname = truncateThaiText($name, 50);
    return SanitizeURL($urlname);
}
...
class PageConfig{
    ...
    public function __construct(){
        ...
        $datas = SelectRows($sql, array());
        // self::$_pageConfig = ConvertSQLResultToArray($datas);
        self::$_pageConfig = $datas;
    }
...
class SystemConfig{
    ...
    private static function COGS($col){
        if(self::$_cogs == null){
            ...
            self::$_cogs = SelectRow($sql, array());
        }
...
function POAdjust($checkoutCode)
    {
        $sql = "update product a
        inner join cart
        on cart.ProductCode = a.ProductCode
        set a.Amount = (a.Amount - cart.QTY)
        where a.Active=1 and cart.CheckOutCode=:CheckOutCode";
        ExecuteSQL($sql,array('CheckOutCode' => $checkoutCode));
    }

    function POIncrease($checkoutCode)
    {
        $sql = "update product a
        inner join cart
        on cart.ProductCode = a.ProductCode
