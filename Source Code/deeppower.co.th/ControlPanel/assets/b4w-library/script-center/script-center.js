...
$.fn.validateUploadVideo = function (previewElt) {
  ...
  if (file.size > maxSize) {
    swal("Invalid file size", "Maximum video size is 500 MB.", "error");
    isValid = false;
  }
  ...
  if (
    previewElt != undefined &&
    $(previewElt).prop("tagName").toUpperCase() == "VIDEO"
  ) {
    var input = $(this);
    var fReader = new FileReader();
...
$.fn.validateUploadSlide = function(previewElt) {
  var isValid = true;
  var file = $(this).get(0).files[0];
  if (!(file.type.match("video/") || file.type.match("image/"))) {
      swal("Invalid file type", "Only video or image file types are allowed.", "error");
      isValid = false;
  }
  if (!isValid) {
      $(this).val("");
  ...
  if(file.type.match("video/")){
      return $(this).validateUploadVideo(previewElt);
  }else if(file.type.match("image/")){
      return $(this).validateUploadImage(previewElt);
  }
  return false;
}
...
function Validate(sender, parent) {
  ...
  if (
    !ValidateEmail(parent) ||
    !ValidateUsername(parent) ||
    !ValidatePassword(parent)
  ) {
    return false;
  }
...
function ValidatePassword(parent) {
  var eltPass =
    parent == undefined
      ? $(".password:visible")
      : $(parent).find(".password:visible");
  var eltRePass =
    parent == undefined
      ? $(".verify-password:visible")
      : $(parent).find(".verify-password:visible");
...
function inputMask() {
  $(".identity-card")
    ...
    .attr("placeholder", "เช่น 1-1234-56789-00-0");
  $(".mobile-phone")
    .mask("99-9999-9999")
    .attr("placeholder", "เช่น 08-8888-8888");
}
...
$(document).ready(function () {
  ...
  $(".jquery-datatable").each(function () {
    ...
    if (table.hasClass("sortable-table")) {
      tableOption.paging = true;
      tableOption.ordering = true;
    }
  ...
  if (urlParams.has("page")) {
    ...
    $(".jquery-datatable").dataTable().fnPageChange(pagenumber, true);
  } else {
    $(".jquery-datatable").dataTable().fnPageChange(0, true);
  }
  ...
  $(".sortable-table tbody").sortable({
    ...
    stop: function (event, ui) {
      ...
      if (
        db_table != undefined &&
        db_column_seq != undefined &&
        db_column_key != undefined &&
        rowItem.length > 0
      ) {
        var rowKeyArr = [];
        ...
        $.ajax({
          url:
            ...
            "/ControlPanel/assets/b4w-framework/SortableItemAPI.php",
          method: "POST",
          data: {
            table: db_table,
  ...
  var sortableDetail = $("<div/>", {
    ...
    colspan: "100%",
    html:
      "<small><i class='fa fa-info-circle'></i><i> คลิกและลากที่รายการเพื่อจัดลำดับใหม่</i></small>" +
  ...
  $(".sortable-table")
    ...
    .before(sortableDetail.clone()); //.after(sortableDetail.clone());
  $("textarea")
    .keyup(function () {
      AutoTextArea($(this)[0]);
    })
