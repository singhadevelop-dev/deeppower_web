...
$sqlPO = "select a.* , b.Name as shipping_name
    from checkout a
    left join delivery_type b on a.DelivieryCode = b.Code
    where CheckOutCode = :CheckOutCode";
$dataPO = SelectRow($sqlPO, array('CheckOutCode' => $POCode));

if(isset($_POST["btnUpdate"])){

    $checkout = SelectRow("select StatusCode from checkout where CheckOutCode = :CheckOutCode ", array('CheckOutCode' => $POCode));
    if($_POST["ddlStatus"] == "CANCEL" && $checkout["StatusCode"] != "CANCEL"){
        POIncrease($POCode);
    }else if($_POST["ddlStatus"] != "CANCEL" && $checkout["ddlStatus"] == "CANCEL"){
        POAdjust($POCode);
    }
    ...
    where CheckOutCode = :CheckOutCode";
    ExecuteSQL($sqlUpdate, array('StatusCode' => $_POST["ddlStatus"], 'CheckOutCode' => $POCode));
                ...
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <p>
                            ...
                            <?php echo $dataPO["Address"]." ".$dataPO["PostCode"] ?>
                        </p>
                        <p>
                            <b>รายละเอียด</b>
                        ...
                        <p>
                            <b>เลือกชำระ</b>
                            <?php echo GetPOPaymentMethodDesc($dataPO["PaymentType"]); ?>
                        </p>
                    </div>
                    <div class="col-sm-6 text-right">
                        <table style="width: 100%">
                            ...
                            <?php if(!empty($dataPO["TaxNumber"])){  ?>
                                <tr>
                                    <td style="text-align: right;"></td>
                                    <td style="text-align: right;">
                                        <h3 class="font-promp">ที่อยู่ส่งใบกำกับภาษี</h3>
                                    </td>  
                                </tr>
                                <tr>
                                    <td style="text-align: right;">
