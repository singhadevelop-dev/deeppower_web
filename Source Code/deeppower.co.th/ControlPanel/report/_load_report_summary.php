...
if(!UserService::_IsAdmin()){
    $whereData .= " and item.ShopCode=:ShopCode ";
    $values["ShopCode"] = UserService::UserCode();
}
if(!empty($month)){
...
if($groupBy == ""){
    ...
    $dataPrd = SelectRowsArray($sqlProduct, $values);
    ...
    $sqlWeb = "select 
                ...
                limit 1
            ";
    $dataWeb = SelectRow($sqlWeb, $values);
...
<?php }else if($groupBy == "CATEGORY"){
    ...
    $sqlShop = "select p.CategoryCode ,cat.CategoryName, cat.Image
            ...
            ";
    $dataCategory = SelectRowsArray($sqlShop, $values);
